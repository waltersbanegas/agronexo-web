...
import { Tractor, PlusCircle, Truck, RefreshCw, Sprout, Scale, DollarSign, MapPin, Locate, Trash2, Edit, CloudRain, Wind, Thermometer, Map as MapIcon, Menu, X, FileDown, Activity, ArrowRightLeft, CheckSquare, Square, Banknote, Syringe, CloudLightning, LayoutDashboard, Warehouse, Factory } from 'lucide-react';

import icon from 'leaflet/dist/images/marker-icon.png';
import iconShadow from 'leaflet/dist/images/marker-shadow.png';
let DefaultIcon = L.icon({ iconUrl: icon, shadowUrl: iconShadow, iconSize: [25, 41], iconAnchor: [12, 41] });
// Icono para Silos (Naranja)
let SiloIcon = L.icon({
...
function App() {
  ...
  const [silos, setSilos] = useState([]); // 🆕 LISTA SILOS
  ...
  const [showModalNuevoSilo, setShowModalNuevoSilo] = useState(false); // 🆕 MODAL SILO
  const [showModalVentaGrano, setShowModalVentaGrano] = useState(false); // 🆕 MODAL VENTA GRANO
  ...
  const cargarTodo = () => {
    ...
    axios.get(`${API_URL}/silos`).then(res => setSilos(res.data)); // 🆕 Cargar Silos
  ...
  // 🆕 REGISTRAR VENTA DE GRANO DESDE SILO
  ...
  // 🆕 LÓGICA MODIFICADA PARA CREAR LOTE O SILO DESDE MAPA
  ...
  const guardarCosecha = (e) => { e.preventDefault(); axios.post(`${API_URL}/nueva_cosecha`, nuevaCosecha).then(() => { setShowModalCosecha(false); cargarTodo(); }); }; // 🆕 ACTUALIZADO
    ...
    <div style={{ fontFamily: 'Segoe UI, sans-serif', backgroundColor: '#f1f5f9', height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{display: 'flex', flex: 1, overflow: 'hidden', position: 'relative'}}>
          ...
          <main style={{flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', background: '#f1f5f9', overflowY: seccion === 'MAPA' ? 'hidden' : 'auto'}}>
              ...
              {seccion === 'MAPA' ? (
                  <div style={{flex: 1, width: '100%', height: '100%', zIndex: 1}}>
                       ...
                       <MapContainer center={[-26.78, -60.85]} zoom={11} style={{ height: '100%', width: '100%' }}>
                          ...
                          {/* 🆕 MENÚ FLOTANTE AL HACER CLICK EN MAPA */}
                          {tempPos && (
                              <Marker position={tempPos}>
                                  <Popup>
                                      <div style={{textAlign:'center', display:'flex', flexDirection:'column', gap:'5px'}}>
                                          <strong>¿Qué quieres crear aquí?</strong>
                                          <button onClick={()=>iniciarCargaDesdeMapa('LOTE')} style={{...btnAzul, padding:'5px', fontSize:'0.8rem'}}>🌱 Nuevo Lote</button>
                                          <button onClick={()=>iniciarCargaDesdeMapa('SILO')} style={{...btnAzul, background:'#f97316', padding:'5px', fontSize:'0.8rem'}}>🏭 Nuevo Silo</button>
                                      </div>
                                  </Popup>
                              </Marker>
                          )}
                          ...
                          {lotes.map(lote => ( 
                              lote.lat && ( 
                                <Marker key={lote.id} position={[lote.lat, lote.lng]} eventHandlers={{ click: () => cargarClima(lote.lat, lote.lng, lote.lote) }}> 
                                    <Popup>
                                        <div style={{textAlign:'center'}}>
                                            <strong style={{fontSize:'1rem'}}>{lote.lote}</strong><br/>{lote.hectareas} Has<br/>
                          ...
                          {silos.map(s => (
                              s.lat && (
                                  <Marker key={`silo-${s.id}`} position={[s.lat, s.lng]} icon={SiloIcon}>
                                      <Popup>
                                          <div style={{textAlign:'center'}}>
                                              <strong style={{fontSize:'1.1rem', color:'#f97316'}}>{s.nombre}</strong><br/>
                                              <span style={{fontSize:'0.9rem', color:'#64748b'}}>{s.tipo} - {s.contenido}</span><br/>
                                              <div style={{fontSize:'1.2rem', fontWeight:'bold', margin:'5px 0'}}>{(s.kilos_actuales / 1000).toFixed(1)} Ton</div>
                                              <button onClick={()=>abrirVentaGrano(s)} style={{...btnAzul, padding:'5px', width:'100%', fontSize:'0.8rem', background:'#16a34a'}}>💰 Vender Stock</button>
                                          </div>
                                      </Popup>
                                  </Marker>
                              )
                  ...
                  <div style={{padding: '20px', paddingBottom: '80px'}}>
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
                            ...
                            <div style={{display:'flex', gap:'10px'}}>
                                {/* 🆕 BOTÓN NUEVO SILO */}
                                <button onClick={() => setShowModalNuevoSilo(true)} style={{...btnOutline, width:'auto', borderColor:'#f97316', color:'#f97316'}}>
                                    <Factory size={20}/> <span style={{display: isMobile ? 'none' : 'inline'}}>Nuevo Silo</span>
                                </button>
                  ...
                  <div style={{padding: '20px', paddingBottom: '80px'}}>
                        {/* SECCIÓN GANADERÍA (Sin cambios visuales) */}
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
              ...
              {/* 🆕 MODAL NUEVO SILO */}
              {showModalNuevoSilo && (
                  <div style={modalBackdrop} onClick={()=>setShowModalNuevoSilo(false)}>
                      <div style={modalContent} onClick={e=>e.stopPropagation()}>
                          <h3 style={{color:'#f97316'}}>Nuevo Silo / Acopio</h3>
                          <form onSubmit={guardarSilo} style={formStyle}>
                              <label style={labelStyle}>Nombre:</label>
                              <input placeholder="Ej: Silobolsa Soja 2024" value={nuevoSilo.nombre} onChange={e=>setNuevoSilo({...nuevoSilo, nombre:e.target.value})} style={inputStyle} required/>
                              
                              <label style={labelStyle}>Tipo:</label>
                              <select style={inputStyle} value={nuevoSilo.tipo} onChange={e=>setNuevoSilo({...nuevoSilo, tipo:e.target.value})}>
                                  <option>SILOBOLSA</option>
                                  <option>CELDA</option>
                                  <option>SILO FIJO</option>
                              </select>

                              <label style={labelStyle}>Contenido (Grano):</label>
                              <select style={inputStyle} value={nuevoSilo.contenido} onChange={e=>setNuevoSilo({...nuevoSilo, contenido:e.target.value})}>
                                  <option>SOJA</option>
                                  <option>MAIZ</option>
                              ...
                              <label style={labelStyle}>Capacidad Max (Tn):</label>
                              <input type="number" placeholder="Ej: 200" value={nuevoSilo.capacidad} onChange={e=>setNuevoSilo({...nuevoSilo, capacidad:e.target.value})} style={inputStyle} required/>
                              
                              <button type="button" onClick={obtenerUbicacion} style={{...btnGris, background:'#0f172a', color:'white', justifyContent:'center'}}><Locate size={18}/> {nuevoSilo.lat ? 'GPS Detectado' : 'Usar GPS'}</button>

                              <button style={{...btnAzul, background:'#f97316'}}>Crear Silo</button>
                          </form>
                      </div>
                  </div>
              )}

              {/* 🆕 MODAL COSECHA ACTUALIZADO (Con destino Silo) */}
              {showModalCosecha && (
                  <div style={modalBackdrop} onClick={()=>setShowModalCosecha(false)}>
                      <div style={modalContent} onClick={e=>e.stopPropagation()}>
                          <div style={{display:'flex', justifyContent:'space-between'}}><h3 style={{color:'#0f172a'}}>Cargar Camión 🚛</h3><button onClick={()=>setShowModalCosecha(false)} style={btnIcon}><X color="#000"/></button></div>
                          <form onSubmit={guardarCosecha} style={formStyle}>
                              <label style={labelStyle}>Kilos Netos:</label>
                              <input type="number" placeholder="Kg" onChange={e=>setNuevaCosecha({...nuevaCosecha, kilos:e.target.value})} style={inputStyle} autoFocus required/>
                              
                              <label style={labelStyle}>Destino de la Carga:</label>
                              <div style={{display:'flex', gap:'10px'}}>
                                  <button type="button" onClick={()=>setNuevaCosecha({...nuevaCosecha, destino:'VENTA'})} style={{...btnOutline, background: nuevaCosecha.destino === 'VENTA' ? '#dbeafe' : 'white', borderColor: nuevaCosecha.destino === 'VENTA' ? '#2563eb' : '#cbd5e1'}}>💰 Venta Directa</button>
                                  <button type="button" onClick={()=>setNuevaCosecha({...nuevaCosecha, destino:'SILO'})} style={{...btnOutline, background: nuevaCosecha.destino === 'SILO' ? '#ffedd5' : 'white', borderColor: nuevaCosecha.destino === 'SILO' ? '#f97316' : '#cbd5e1'}}>🏭 Acopio / Silo</button>
                              </div>

                              {nuevaCosecha.destino === 'SILO' && (
                                  <div style={{marginTop:'10px'}}>
                                      <label style={labelStyle}>Seleccionar Silo:</label>
                                      <select style={inputStyle} onChange={e=>setNuevaCosecha({...nuevaCosecha, silo_id:e.target.value})} required>
                                          <option value="">-- Elegir Silo --</option>
                                          {silos.map(s => (
                                              <option key={s.id} value={s.id}>{s.nombre} ({s.contenido})</option>
                                          ))}
                                      </select>
                                  </div>
                              )}
              ...
              {/* 🆕 MODAL VENTA DE GRANO */}
              {showModalVentaGrano && (
                  <div style={modalBackdrop} onClick={()=>setShowModalVentaGrano(false)}>
                      <div style={modalContent} onClick={e=>e.stopPropagation()}>
                          <div style={{display:'flex', justifyContent:'space-between'}}><h3 style={{color:'#16a34a'}}>Vender Stock 💰</h3><button onClick={()=>setShowModalVentaGrano(false)} style={btnIcon}><X color="#000"/></button></div>
                          <form onSubmit={confirmarVentaGrano} style={formStyle}>
                              <div style={{background:'#f0fdf4', padding:'10px', borderRadius:'8px', fontSize:'0.9rem', color:'#166534', border:'1px solid #bbf7d0', marginBottom:'10px'}}>
                                  Vendiendo {ventaGrano.tipo_grano} desde Silo.
                              </div>
                              <label style={labelStyle}>Comprador:</label>
                              <input placeholder="Ej: AFA" value={ventaGrano.comprador} onChange={e=>setVentaGrano({...ventaGrano, comprador:e.target.value})} style={inputStyle} required/>
                              
                              <label style={labelStyle}>Kilos a Vender:</label>
                              <input type="number" placeholder="Kg" value={ventaGrano.kilos} onChange={e=>setVentaGrano({...ventaGrano, kilos:e.target.value})} style={inputStyle} required/>
                              
                              <label style={labelStyle}>Precio Total ($):</label>
                              <input type="number" placeholder="$" value={ventaGrano.precio_total} onChange={e=>setVentaGrano({...ventaGrano, precio_total:e.target.value})} style={inputStyle} required/>

                              <button style={{...btnAzul, background:'#16a34a'}}>Confirmar Venta</button>
                          </form>
                      </div>
                  </div>
              )}

              {/* RESTO DE MODALES IGUALES... (Lote, Animal, Pesaje, Gasto, Lluvia, Sanidad) */}
              {/* [AQUÍ MANTENGO LOS DEMÁS MODALES SIN CAMBIOS PARA AHORRAR ESPACIO EN LA RESPUESTA, PERO EN EL CÓDIGO FINAL DEBEN ESTAR TODOS] */}
