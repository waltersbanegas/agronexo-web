...
class Animal(db.Model):
    ...
    estado_general = db.Column(db.String(50), default='ACTIVO') # ACTIVO, BAJA

class Baja(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    animal_id = db.Column(db.Integer, db.ForeignKey('animal.id'))
    fecha = db.Column(db.DateTime, default=datetime.utcnow)
    motivo = db.Column(db.String(50)) # VENTA, ROBO, MUERTE, CONSUMO
...
def safe_int(val):
    try: return int(float(str(val))) if val else None
    except: return None
...
def reset_fabrica():
    try:
        db.drop_all()
        db.create_all()
        return jsonify({"mensaje": "SISTEMA RESTAURADO Y LIMPIO"})
...
def resumen_general():
    try:
        ...
        # Margen: (Ventas de Grano + Bajas por Venta) - Gastos
        ...
        margen = (ventas_grano + ventas_hacienda) - gastos
        
        return jsonify({ "cabezas": cabezas, "hectareas": has, "stock_granos": grano, "gastos_mes": gastos, "margen_mes": margen, "lluvia_mes": 0 })
...
def get_animales():
    try:
        res = []
        # Solo devolver animales ACTIVOS
...
def registrar_baja():
    try:
        ...
        motivo = d.get('motivo')
        valor = safe_float(d.get('precio')) # Solo si es venta
        detalle = d.get('detalle', '')
        
        # Guardar en Historial de Bajas
        baja = Baja(animal_id=aid, motivo=motivo, detalle=detalle, valor=valor, fecha=datetime.utcnow())
        db.session.add(baja)
        
        # Marcar animal como NO ACTIVO
        ...
        db.session.commit()
        return jsonify({"mensaje": f"Baja por {motivo} registrada"})
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def nc(): d=request.json; nl=Lote(nombre=d['nombreLote'], hectareas=safe_float(d['hectareas']), latitud=safe_float(d.get('lat')), longitud=safe_float(d.get('lng'))); db.session.add(nl); db.session.commit(); db.session.add(ContratoCampo(lote_id=nl.id, propietario=d['propietario'], tipo=d['tipo'], porcentaje_dueno=safe_float(d.get('porcentaje')))); db.session.commit(); return jsonify({"msg":"OK"})

@app.route('/api/mover_hacienda', methods=['POST'])
def mv(): d=request.json; dest=safe_int(d.get('lote_destino_id')); [setattr(Animal.query.get(aid), 'lote_actual_id', dest) for aid in d.get('animales_ids', []) if Animal.query.get(aid)]; db.session.commit(); return jsonify({"msg":"OK"})
...
def gm(): d=request.json; [db.session.add(Gasto(fecha=datetime.utcnow(), concepto=d['concepto'], monto=safe_float(d['monto']), animal_id=a.id)) for a in Animal.query.all()]; db.session.commit(); return jsonify({"msg":"OK"})
...
def rll(): d=request.json; db.session.add(Lluvia(lote_id=safe_int(d['lote_id']), milimetros=safe_float(d['milimetros']), fecha=datetime.utcnow())); db.session.commit(); return jsonify({"msg":"OK"})
...
# --- ARRANQUE Y AUTOREPARACIÓN ---
if __name__ == '__main__':
    with app.app_context():
        try:
            # Intenta crear tablas. Si falla por corrupción, borra todo y reintenta.
            db.create_all()
        except:
            print("⚠️ BASE DE DATOS CORRUPTA DETECTADA. INICIANDO PROTOCOLO DE RESET TOTAL...")
            db.drop_all()
            db.create_all()
