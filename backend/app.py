...
# ⚠️ MODELO DE VENTA (IMPORTANTE: MANTENER PARA COMPATIBILIDAD) ⚠️
...
def reset_fabrica():
    try:
        db.drop_all()
        db.create_all()
        return jsonify({"mensaje": "SISTEMA RESTAURADO"})
    except Exception as e:
        db.session.rollback() # ⚠️ DESTRABAR
...
def resumen_general():
    try:
        ...
        ventas_grano = db.session.query(func.sum(VentaGrano.precio_total)).filter(extract('month', VentaGrano.fecha) == mes).scalar() or 0
        ventas_hacienda = db.session.query(func.sum(Baja.valor)).filter(extract('month', Baja.fecha) == mes, Baja.motivo == 'VENTA').scalar() or 0
        
        return jsonify({ "cabezas": cabezas, "hectareas": round(has, 1), "stock_granos": round(grano, 0), "gastos_mes": round(gastos, 2), "margen_mes": round((ventas_grano + ventas_hacienda) - gastos, 2), "lluvia_mes": 0 })
...
def get_animales():
    try:
        ...
        for a in Animal.query.filter_by(estado_general='ACTIVO').all():
            ...
            res.append({ "id": a.id, "caravana": a.caravana, "categoria": a.categoria, "raza": a.raza, "peso_actual": peso, "estado_reproductivo": a.estado_reproductivo, "lote_actual_id": a.lote_actual_id, "ubicacion": loc })
        return jsonify(res)
...
def detalle_animal(id):
    try:
        ...
        return jsonify({ "id": a.id, "caravana": a.caravana, "categoria": a.categoria, "raza": a.raza, "rfid": a.rfid or "", "historial_pesos": pesos, "historial_gastos": gastos, "historial_repro": evs, "estado_reproductivo": a.estado_reproductivo })
    except: return jsonify({"error": "Error"}), 500

# --- RUTAS DE ACCIÓN BLINDADAS (CON ROLLBACK) ---
...
def na():
    try:
        d=request.json; db.session.add(Animal(caravana=d['caravana'], rfid=d.get('rfid'), raza=d['raza'], categoria=d['categoria'])); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error": str(e)}), 500
...
def registrar_baja():
    try:
        ...
        baja = Baja(animal_id=aid, motivo=d.get('motivo'), detalle=d.get('detalle', ''), valor=safe_float(d.get('precio')), fecha=datetime.utcnow())
        db.session.add(baja)
        a = Animal.query.get(aid)
        if a: a.estado_general = 'BAJA'; a.lote_actual_id = None
        db.session.commit(); return jsonify({"mensaje": "Baja OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error": str(e)}), 500

@app.route('/api/registrar_venta', methods=['POST'])
def registrar_venta():
    # MANTENIDA POR COMPATIBILIDAD PERO REDIRIGE A BAJA INTERNAMENTE
    try:
        d=request.json; aid=safe_int(d.get('animal_id'))
        baja = Baja(animal_id=aid, motivo='VENTA', detalle=d.get('comprador'), valor=safe_float(d.get('precio')), fecha=datetime.utcnow())
        db.session.add(baja)
        a=Animal.query.get(aid)
        if a: a.estado_general='BAJA'; a.lote_actual_id=None
        db.session.commit(); return jsonify({"mensaje":"Venta OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        if a:
            if d['tipo'] == 'INSEMINACION': a.estado_reproductivo='INSEMINADA'
                ...
                try: db.session.add(Animal(caravana=f"CRIA-{str(int(time.time()))[-4:]}", categoria='Ternero', raza=a.raza, lote_actual_id=a.lote_actual_id))
                except: pass
        db.session.commit(); return jsonify({"mensaje":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error": str(e)}), 500
...
def liq():
    try:
        res=[]; hoy=datetime.utcnow()
        for l in Lote.query.all():
            c=ContratoCampo.query.filter_by(lote_id=l.id).first()
            res.append({"id":c.id if c else l.id*999, "lote_id":l.id, "lote":l.nombre, "hectareas":l.hectareas, "tipo":c.tipo if c else "S/D", "propietario":c.propietario if c else "-", "porcentaje":c.porcentaje_dueno if c else 0, "lat":l.latitud, "lng":l.longitud, "total_cosechado":0, "total_gastos":0, "lluvia_mes":0})
        return jsonify(res)
    except: return jsonify([])
...
def ea(id):
    try:
        a=Animal.query.get(id); d=request.json; a.caravana=d['caravana']; a.rfid=d.get('rfid'); a.categoria=d['categoria']; a.raza=d['raza']; db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500

@app.route('/api/nuevo_pesaje', methods=['POST'])
def np():
    try:
        d=request.json; db.session.add(Peso(animal_id=d['animal_id'], kilos=safe_float(d['kilos']), fecha=datetime.utcnow())); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500

@app.route('/api/gasto_masivo', methods=['POST'])
def gm():
    try:
        d=request.json; db.session.add(Gasto(fecha=datetime.utcnow(), concepto=d['concepto'], monto=safe_float(d['monto']))); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500

@app.route('/api/nuevo_silo', methods=['POST'])
def ns():
    try:
        d=request.json; db.session.add(Silo(nombre=d['nombre'], tipo=d['tipo'], contenido=d['contenido'], capacidad=safe_float(d['capacidad']))); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500
...
def vg(): 
    try:
        d=request.json; db.session.add(VentaGrano(comprador=d['comprador'], tipo_grano=d['tipo_grano'], kilos=safe_float(d['kilos']), precio_total=safe_float(d['precio_total']), origen=d['origen'], silo_id=safe_int(d.get('silo_id')))); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500
@app.route('/api/nueva_cosecha', methods=['POST'])
def nco(): 
    try:
        d=request.json; db.session.add(Cosecha(lote_id=safe_int(d['lote_id']), kilos_totales=safe_float(d['kilos']))); db.session.commit(); return jsonify({"msg":"OK"})
    except Exception as e: db.session.rollback(); return jsonify({"error":str(e)}), 500
